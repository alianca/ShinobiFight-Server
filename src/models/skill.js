...
Skill.prototype.compute_damage = function(attacker, attacked) {
    if (this.blocked) {
        ...
        var success = (this.precision + attacker_st.precision + attacker.status[this.type]) / 3 - attacked_st.dodge;
        var critical = (this.critical + attacker_st.critical / 3) * attacker.status[this.type];
        success = success > Math.random() * 100;
        critical = critical > Math.random() * 100;
